...
public class ApplicationController {
    ...
    @Operation(summary = "지원자 목록 조회", description = "전체 또는 특정 공고(companyId=공고 ID)의 지원자 목록을 조회합니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공"),
            @ApiResponse(responseCode = "401", description = "인증되지 않은 사용자"),
    ...
    public List<ApplicationJpaEntity> queryApplicants(@RequestParam(required = false) Long companyId) {
        return queryApplicantsService.execute(companyId);
    }
